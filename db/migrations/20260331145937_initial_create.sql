...
UPDATE
,
    DELETE ON TABLES TO app_system;

ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE,
SELECT
    ON SEQUENCES TO app_system;
...
CREATE TABLE IF NOT EXISTS users (
    ...
    CONSTRAINT fk_users_identity_id FOREIGN KEY (identity_id) REFERENCES identities (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS user_accounts (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_users_accounts_user_id FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE,
    CONSTRAINT check_user_accounts_provider_id CHECK (
        provider_id IN ('credential', 'google')
    )
);
...
CREATE TABLE IF NOT EXISTS customers (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_customers_identity_id FOREIGN KEY (identity_id) REFERENCES identities (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS customer_accounts (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_customer_accounts_customer_id FOREIGN KEY (customer_id) REFERENCES customers (id) ON DELETE CASCADE,
    CONSTRAINT check_customer_accounts_provider_id CHECK (
        provider_id IN ('credential', 'google')
    )
);
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_sessions_identity_id FOREIGN KEY (identity_id) REFERENCES identities (id) ON DELETE CASCADE,
    CONSTRAINT check_sessions_service CHECK (
        service IN (
            'admin_panel',
            'merchant_panel',
            'buyer_platform'
...
CREATE TABLE IF NOT EXISTS members (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_members_identity_id FOREIGN KEY (identity_id) REFERENCES identities (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS roles (
    ...
    CONSTRAINT check_roles_organization_type CHECK (
...
CREATE TABLE IF NOT EXISTS member_roles (
    ...
    CONSTRAINT fk_member_roles_assigned_by FOREIGN KEY (assigned_by) REFERENCES members (id) ON DELETE CASCADE
);
...
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE USAGE,
SELECT
    ON SEQUENCES
FROM
    app_user;
