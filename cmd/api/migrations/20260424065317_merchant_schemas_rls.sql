...
ALTER TABLE
    inventories ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    categories FORCE ROW LEVEL SECURITY;
...
ALTER TABLE
    inventories FORCE ROW LEVEL SECURITY;

CREATE
OR REPLACE FUNCTION is_product_active(product_id uuid) RETURNS bool AS
$$
SELECT
    EXISTS (
        SELECT
...
CREATE
OR REPLACE FUNCTION is_organization_warehouse(p_warehouse_id bigint) RETURNS bool AS
$$
SELECT
    EXISTS (
        SELECT
...
CREATE POLICY org_isolation_read ON categories FOR
SELECT
    USING (
        is_active = TRUE
...
CREATE POLICY org_isolation_write ON categories FOR
INSERT
    WITH CHECK (
...
CREATE POLICY org_isolation_update ON categories FOR
UPDATE
    USING (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_delete ON categories FOR DELETE USING (
    organization_id = current_org_id()
    OR is_platform_admin()
);

CREATE POLICY org_isolation_read ON products FOR
SELECT
    USING (
        STATUS = 'active'
        OR organization_id = current_org_id()
...
CREATE POLICY org_isolation_write ON products FOR
INSERT
    WITH CHECK (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_update ON products FOR
UPDATE
    USING (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_delete ON products FOR DELETE USING (
    organization_id = current_org_id()
    OR is_platform_admin()
);

CREATE POLICY org_isolation_read ON product_variants FOR
SELECT
    USING (
...
CREATE POLICY org_isolation_write ON product_variants FOR
INSERT
    WITH CHECK (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_update ON product_variants FOR
UPDATE
    USING (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_delete ON product_variants FOR DELETE USING (
    organization_id = current_org_id()
    OR is_platform_admin()
);
...
CREATE POLICY org_isolation_write ON warehouses FOR
INSERT
    WITH CHECK (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_update ON warehouses FOR
UPDATE
    USING (
        organization_id = current_org_id()
        OR is_platform_admin()
...
CREATE POLICY org_isolation_delete ON warehouses FOR DELETE USING (
    organization_id = current_org_id()
    OR is_platform_admin()
);

CREATE POLICY org_isolation_read ON inventories FOR
SELECT
    USING (
...
CREATE POLICY org_isolation_write ON inventories FOR
INSERT
    WITH CHECK (
        is_organization_warehouse(warehouse_id)
        OR is_platform_admin()
...
CREATE POLICY org_isolation_update ON inventories FOR
UPDATE
    USING (
        is_organization_warehouse(warehouse_id)
        OR is_platform_admin()
...
CREATE POLICY org_isolation_delete ON inventories FOR DELETE USING (
    is_organization_warehouse(warehouse_id)
    OR is_platform_admin()
);
...
ALTER TABLE
    inventories NO FORCE ROW LEVEL SECURITY;

ALTER TABLE
    warehouses NO FORCE ROW LEVEL SECURITY;
...
ALTER TABLE
    categories NO FORCE ROW LEVEL SECURITY;

ALTER TABLE
    inventories DISABLE ROW LEVEL SECURITY;

ALTER TABLE
    warehouses DISABLE ROW LEVEL SECURITY;
...
ALTER TABLE
    categories DISABLE ROW LEVEL SECURITY;
